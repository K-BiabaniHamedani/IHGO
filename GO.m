function [gbestX,gbestweight,gbestfitness,gbestfrequencies,gbesthistory_weight,gbesthistory_fitness,FEs,e1,e2,e3,P1,P2,P3] = GO(nodes,elements,f,popsize,dimension,xmin,xmax,MaxFEs,run_id)

%% Parameter setting
Iter = 0; FEs = 0; P1 = 5; P2 = 0.001; P3 = 0.3;
...
frequency = zeros(popsize,5);
newfrequency = zeros(popsize,5);
...
for i = 1:popsize
    FEs = FEs+1;
    [x(i,:),weight(i,1),frequency(i,:),fitness(i,1),e1,e2,e3] = fobj(nodes,x(i,:),elements,f,VarMin,VarMax,FEs,MaxFEs);
    if fitness(i,1) < gbestfitness
        gbestfitness = fitness(i,1);
        gbestweight = weight(i,1);
        gbestX = x(i,:);
        gbestfrequencies = frequency(i,:);
    end
    gbesthistory_fitness(FEs,1) = gbestfitness;
    gbesthistory_weight(FEs,1) = gbestweight;
    disp(['NRun = ' num2str((run_id)) '; NIter = ' num2str(Iter) '; NFEs = ' num2str(FEs) '; minFit = ' num2str(gbesthistory_weight(FEs,1)) '; minPFit = ' num2str(gbesthistory_fitness(FEs,1))]);
end
...
while FEs < MaxFEs
    ...
    for i = 1:popsize
        ...
        FEs = FEs+1;
        [newx(i,:),newweight,newfrequency(i,:),newfitness,e1,e2,e3] = fobj(nodes,newx(i,:),elements,f,VarMin,VarMax,FEs,MaxFEs);
